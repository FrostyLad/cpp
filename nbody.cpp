/* Johnston, Braden, 20005898, Assignment 3, 159.341 Semester 1 2023
*/
/* C++ N-body simluation using OpenMP API for parallelization in the hefty for loops inside the update function
 * and for atomic modification of acceleration values.
 * Note that I have added a flag to include OpenMP inside the makefile, which was uploaded alongside this.
 * My apologies for such a late submission, I hope that everything is going well for you*/
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for(int j = i+1; j < N; ++j) {
            ...
            if(d2 > min2) {
                ...
                // Add to acceleration
                #pragma omp atomic
                acc[i].x += (u.x * f / bodies[i].mass);
                #pragma omp atomic
                acc[i].y += (u.y * f / bodies[i].mass);
                #pragma omp atomic
                acc[j].x -= (u.x * f / bodies[j].mass);
                #pragma omp atomic
                acc[j].y -= (u.y * f / bodies[j].mass);
            }
        }
    }
...
int main() {
    ...
    printf( " Submitted by: Braden Johnston - 20005898 \n");
    printf( "------------------------------------------\n");
